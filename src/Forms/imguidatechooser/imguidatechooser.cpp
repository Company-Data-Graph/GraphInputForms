...
int getMaxMonthWidthIndex(char monthNames[12][16])
{
	int maxMonthWidthIndex = -1;
	float maxMonthWidth = 0;
	for (uint8_t i = 0; i < 12; ++i)
	{
		float width = ImGui::CalcTextSize(&monthNames[i][0]).x;
		if (maxMonthWidth < width)
		{
			maxMonthWidth = width;
	 ...
	 const char* downArrowString)
{
	...
	constexpr const uint8_t monthCount = 12;

	static char dayNames[][nameMaxLength] = {"S", "M", "T", "W", "T", "F", "S"};
	static char monthNames[monthCount][nameMaxLength]
		 = {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"};
	static int maxMonthWidthIndex = getMaxMonthWidthIndex(monthNames);

	ImGuiStyle& style = g.Style;

	const char* arrowLeft = leftArrow != nullptr ? leftArrow : "<";
	const char* arrowRight = rightArrow != nullptr ? rightArrow : ">";
	...
	const float widthRequiredByCalendar = 2.f * arrowLeftWidth + 2.f * arrowRightWidth
										  + ImGui::CalcTextSize(monthNames[maxMonthWidthIndex]).x + ImGui::CalcTextSize("9999").x + 50.f
										  + widthAdder;
	...
	// const ImRect popup_rect(ImVec2(frame_bb.Min.x+popup_off_x+ImGui::GetScrollX(), frame_bb.Max.y+ImGui::GetScrollY()),
	// ImVec2(frame_bb.Max.x+popup_off_x + ImGui::GetScrollX(), frame_bb.Max.y + popup_height + ImGui::GetScrollY()));
	...
	if (g.IO.MouseClicked[ImGuiMouseButton_Right])
	{
		d = GetCurrentDate();
		d.tm_mday = 1;
		RecalculateDateDependentFields(d);
	...
	for (int dw = 0; dw < 7; dw++)
	{
		...
		int curDay
			 = dw
			   - d.tm_wday;	 // tm_wday is in [0,6]. For this to work here d must point to the first day of the month: i.e.: d.tm_mday = 1;
...
// Legacy code that used to work much better than the above code (but now new child windows in Dear Imgui aren't drawn above the current
// window, so this code doesn't work anymore)
bool DateChooser(const char* label,
	 tm& dateOut,
	 const char* dateFormat,
	 bool closeWhenMouseLeavesIt,
	 ...
	 const char* downArrowString)
{
	...
	static char* pMonthNames[12]
		 = {&monthNames[0][0], &monthNames[1][0], &monthNames[2][0], &monthNames[3][0], &monthNames[4][0], &monthNames[5][0],
			  &monthNames[6][0], &monthNames[7][0], &monthNames[8][0], &monthNames[9][0], &monthNames[10][0], &monthNames[11][0]};
	...
	if (strlen(dayNames[0]) == 0)
	{
		...
		for (int i = 0; i < 12; i++)
		{
			...
			{
				static const char* fallbacks[12] = {"January", "February", "March", "April", "May", "June", "July", "August", "September",
	...
	RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max,
		 GetColorU32(hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button), true, style.FrameRounding);  // FIXME-ROUNDING
	RenderArrow(window->DrawList, ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y) + style.FramePadding, GetColorU32(ImGuiCol_Text),
		 ImGuiDir_Down);
	...
	if (isOpen)
	{
		...
		const float popup_height
			 = (label_size.y + 2 * style.ItemSpacing.y) * ImMin(items_count, height_in_items) + (style.FramePadding.y * 3);
		const ImRect popup_rect(ImVec2(frame_bb.Min.x + popup_off_x + ImGui::GetScrollX(), frame_bb.Max.y + ImGui::GetScrollY()),
			 ImVec2(frame_bb.Max.x + popup_off_x + ImGui::GetScrollX(), frame_bb.Max.y + popup_height + ImGui::GetScrollY()));
		...
		const ImGuiWindowFlags flags
			 = /*ImGuiWindowFlags_ComboBox |*/ ImGuiWindowFlags_NoScrollbar
			   | ImGuiWindowFlags_NoScrollWithMouse;  // | ImGuiWindowFlags_Modal;    // Adding ImGuiWindowFlags_Modal seems to work too...
		ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, style.FramePadding);
		ImGui::PushStyleColor(ImGuiCol_ChildBg, style.Colors[ImGuiCol_PopupBg]);
...
// #       undef  IMGUIVARIOUSCONTROLS_H_
		...
		{
			if (previousOpen != popupMonthParams.open)
				ImGui::SetNextWindowPos(menupos);
			const int selectedEntry
				 = ImGui::PopupMenuSimple(popupMonthParams, (const char**)pMonthNames, 12, 6, false, NULL, upArrowString, downArrowString);
			if (selectedEntry >= 0)
			{
				d.tm_mon = selectedEntry;
				RecalculateDateDependentFields(d);
			}
		}
#endif	// IMGUIVARIOUSCONTROLS_H_
		...
		ImGui::SameLine(
			 ImGui::GetWindowWidth() - yearPartWidth - ImGui::GetStyle().WindowPadding.x - ImGui::GetStyle().ItemSpacing.x * 4.f);
		...
		if (popupYearParams.open)
		{
			...
			for (int i = 0; i < 20; i++)
			{
				sprintf(&years[i][0], "%d", 1900 + d.tm_year - 10 + i);
				pyears[i] = &years[i][0];
			}
			if (previousOpen != popupYearParams.open)
				ImGui::SetNextWindowPos(menupos);
			const int selectedEntry
				 = ImGui::PopupMenuSimple(popupYearParams, (const char**)pyears, 20, 6, false, NULL, upArrowString, downArrowString);
			if (selectedEntry >= 0)
			{
				if (selectedEntry + d.tm_year - 10 >= 0)
				{
					d.tm_year += selectedEntry - 10;
					RecalculateDateDependentFields(d);
				}
			}
		}
		...
		const static int numDaysPerMonth[12] = {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31};
		int maxDayOfCurMonth
			 = numDaysPerMonth[d.tm_mon];  // This could be calculated only when needed (but I guess it's fast in any case...)
		if (maxDayOfCurMonth == 28)
		{
			const int year = d.tm_year + 1900;
			const bool bis = ((year % 4) == 0) && ((year % 100) != 0 || (year % 400) == 0);
		...
		for (int dw = 0; dw < 7; dw++)
		{
			...
			ImGui::Spacing();
			//-----------------------------------------------------------------------
			int curDay = dw - d.tm_wday;  // tm_wday is in [0,6]. For this to work here d must point to the first day of the month: i.e.:
										  // d.tm_mday = 1;
			for (int row = 0; row < 7; row++)
			{
				int cday = curDay + 7 * row;
...
void TestDateChooser(const char* dateFormat,
	 bool closeWhenMouseLeavesIt,
	 const char* leftArrow,
	 const char* rightArrow,
	 const char* upArrowString,
	 const char* downArrowString)
{
	...
	if (DateChooser(
			 "DateChooser##id", date, dateFormat, closeWhenMouseLeavesIt, NULL, leftArrow, rightArrow, upArrowString, downArrowString))
	{
