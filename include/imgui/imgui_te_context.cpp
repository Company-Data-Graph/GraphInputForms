...
struct ImGuiTestContextDepthScope
{
	...
	ImGuiTestContextDepthScope(ImGuiTestContext* ctx)
	{
		TestContext = ctx;
		TestContext->ActionDepth++;
	}
...
inline const char* GetActionName(ImGuiTestAction action)
{
	switch (action)
	{
	case ImGuiTestAction_Unknown:
		return "Unknown";
	case ImGuiTestAction_Hover:
		return "Hover";
	case ImGuiTestAction_Click:
		return "Click";
	case ImGuiTestAction_DoubleClick:
		return "DoubleClick";
	...
	case ImGuiTestAction_Close:
		return "Close";
	case ImGuiTestAction_Input:
		return "Input";
	case ImGuiTestAction_NavActivate:
		return "NavActivate";
...
void ImGuiTestContext::LogBasicUiState()
{
	...
	LogDebug("Hovered: 0x%08X (\"%s\"), Active:  0x%08X(\"%s\")", item_hovered_id,
		 item_hovered_info->ID != 0 ? item_hovered_info->DebugLabel : "", item_active_id,
		 item_active_info->ID != 0 ? item_active_info->DebugLabel : "");
}
...
{
	while (FrameCount < frame_count) ImGuiTestEngine_Yield(Engine);
}
...
bool ImGuiTestContext::SuspendTestFunc(const char* file, int line)
{
	...
	while (Test->Status == ImGuiTestStatus_Suspended && !Abort) Yield();
...
// Sleep for a given clock time from the point of view of the Dear ImGui context, without affecting wall clock time of the running
// application.
...
ImGuiTestRef ImGuiTestContext::GetRef() { return RefID; }
...
ImGuiID ImGuiTestContext::GetIDByInt(int n) { return ImHashData(&n, sizeof(n), GetID(RefID)); }

ImGuiID ImGuiTestContext::GetIDByInt(int n, ImGuiTestRef seed_ref) { return ImHashData(&n, sizeof(n), GetID(seed_ref)); }

ImGuiID ImGuiTestContext::GetIDByPtr(void* p) { return ImHashData(&p, sizeof(p), GetID(RefID)); }

ImGuiID ImGuiTestContext::GetIDByPtr(void* p, ImGuiTestRef seed_ref) { return ImHashData(&p, sizeof(p), GetID(seed_ref)); }
#endif
...
bool ImGuiTestContext::CaptureScreenshot(int capture_flags)
{
	...
	if (can_capture)
		...
		LogWarning("Skipped saving '%s' (%d*%d pixels) (enable in 'Misc->Options')", args->InOutputFile, (int)args->OutImageSize.x,
			 (int)args->OutImageSize.y);
	return ret;
#else
	IM_UNUSED(args);
...
void ImGuiTestContext::CaptureReset() { *CaptureArgs = ImGuiCaptureArgs(); }
...
ImGuiID ImGuiTestContext::ItemInfoHandleWildcardSearch(const char* wildcard_prefix_start,
	 ...
	 const char* wildcard_suffix_start)
{
	...
	// Wildcard matching requires item to be visible, because clipped items are unaware of their labels. Try panning through entire window,
	// searching for target item. (Scrollbar position restoration in theory may be desirable, however it interferes with typical use of
	// found item)
	// FIXME-TESTS: This doesn't recurse properly into each child..
	// FIXME: Down the line if we refactor ItemAdd() return value to distinguish render-clipping vs logic-clipping etc, we should instead
	// temporarily enable a "no clip" mode without the need for scrolling.
	if (task->OutItemId == 0)
	{
	...
	// FIXME: InFilterItemStatusFlags is intentionally not cleared here, because it is set in ItemAction() and reused in later calls to
	// ItemInfo() to resolve ambiguities.
	task->InPrefixId = 0;
	task->InSuffix = task->InSuffixLastItem = NULL;
	task->InSuffixLastItemHash = 0;
	task->InSuffixDepth = 0;
	task->OutItemId = 0;  // -V1048   // Variable 'OutItemId' was assigned the same value. False-positive, because value of OutItemId could
						  // be modified from other thread during ImGuiTestEngine_Yield() call.
...
ImGuiTestItemInfo* ImGuiTestContext::ItemInfo(ImGuiTestRef ref, ImGuiTestOpFlags flags)
{
	...
	if (const char* p = ref.Path ? strstr(ref.Path, "**/") : NULL)
	{
		...
		// We could add a simple helpers that would iterate the strings, handling inhibitors, and let you check if a given characters is
		// inhibited or not.
	...
	while (full_id && retries < max_retries)
	{
		...
		// While a window is appearing it is likely to be resizing and items moving. Wait an extra frame for things to settle. (FIXME: Could
		// use another source e.g. Hidden? AutoFitFramesX?)
...
ImGuiTestItemInfo* ImGuiTestContext::ItemInfoOpenFullPath(ImGuiTestRef ref, ImGuiTestOpFlags flags)
{
	...
	ImGuiTestItemInfo* item
		 = ItemInfo(ref, (can_open_full_path ? ImGuiTestOpFlags_NoError : ImGuiTestOpFlags_None) | (flags & ImGuiTestOpFlags_NoError));
	if (item->ID != 0)
		return item;
	...
	for (const char* parent_end = strstr(ref.Path, "/"); parent_end != NULL; parent_end = strstr(parent_end + 1, "/"))
	{
		...
		if (parent_item->ID != 0 && (parent_item->StatusFlags & ImGuiItemStatusFlags_Openable) != 0
			 && (parent_item->StatusFlags & ImGuiItemStatusFlags_Opened) == 0)
		{
...
// - Likely you may want to feed the return value into SetRef(): e.g. 'ctx->SetRef(item->ID)' or
// 'ctx->SetRef(WindowInfo("//Window/Child")->ID);' Todos:
...
ImGuiTestItemInfo* ImGuiTestContext::WindowInfo(ImGuiTestRef ref, ImGuiTestOpFlags flags)
{
	...
	if (ref.ID != 0)
	{
		...
		{
			LogEx(log_level, 0,
				 "WindowInfo: error: cannot find window by ID!");  // FIXME: What if we want to query a not-yet-existing window by ID?
			return ItemInfoNull();
		}
...
void ImGuiTestContext::ScrollTo(ImGuiTestRef ref, ImGuiAxis axis, float scroll_target, ImGuiTestOpFlags flags)
{
	...
	while (!Abort)
	{
		...
		const float scroll_speed
			 = (EngineIO->ConfigRunSpeed == ImGuiTestRunSpeed_Fast) ? FLT_MAX : ImFloor(EngineIO->ScrollSpeed * g.IO.DeltaTime + 0.99f);
...
void ImGuiTestContext::ScrollToItemX(ImGuiTestRef ref) { ScrollToItem(ref, ImGuiAxis_X); }

void ImGuiTestContext::ScrollToItemY(ImGuiTestRef ref) { ScrollToItem(ref, ImGuiAxis_Y); }
...
void ImGuiTestContext::ScrollToTabItem(ImGuiTabBar* tab_bar, ImGuiID tab_id)
{
	...
	{
		MouseMove("##<");
		for (int i = 0; i < selected_tab_index - target_tab_index; ++i) MouseClick(0);
	}
	else
	{
		MouseMove("##>");
		for (int i = 0; i < target_tab_index - selected_tab_index; ++i) MouseClick(0);
	}
...
void ImGuiTestContext::NavMoveTo(ImGuiTestRef ref)
{
	...
	while (g.NavMoveSubmitted) Yield();
	Yield();
...
void ImGuiTestContext::MouseMove(ImGuiTestRef ref, ImGuiTestOpFlags flags)
{
	...
	if (!Abort && !(flags & ImGuiTestOpFlags_NoCheckHoveredId))
	{
		...
		if (!is_hovered_item)
			for (ImGuiWindow* hovered_window = g.HoveredWindow; hovered_window != NULL && !is_hovered_window;
				 hovered_window = hovered_window->ParentWindow)
		...
		if (!is_hovered_item && !is_hovered_window)
		{
			if (!(window->Flags & ImGuiWindowFlags_NoResize) && !(flags & ImGuiTestOpFlags_IsSecondAttempt))
			{
				...
				for (int n = 0; n < 2; n++) is_resize_corner |= (hovered_id == ImGui::GetWindowResizeID(window, n));
#else
				for (int n = 0; n < 2; n++) is_resize_corner |= (hovered_id == ImGui::GetWindowResizeCornerID(window, n));
#endif
				if (is_resize_corner)
				{
			...
			Str256f error_message(
				 ...
				 desc.c_str(), item->ID, item->Window ? item->Window->Name : "<NULL>", pos.x, pos.y, hovered_id,
				 g.HoveredWindow ? g.HoveredWindow->Name : "", pos_old.x, pos_old.y, pos_new.x, pos_new.y,
				 (pos_old.x == pos_new.x && pos_old.y == pos_new.y) ? "Same" : "Changed", size_old.x, size_old.y, size_new.x, size_new.y,
				 (size_old.x == size_new.x && size_old.y == size_new.y) ? "Same" : "Changed");
...
bool ImGuiTestContext::WindowTeleportToMakePosVisible(ImGuiTestRef ref, ImVec2 pos)
{
	...
	if (!visible_r.Contains(pos))
	{
		...
		ImVec2 delta;
		delta.x = (pos.x < visible_r.Min.x)	  ? (visible_r.Min.x - pos.x + pad)
				  : (pos.x > visible_r.Max.x) ? (visible_r.Max.x - pos.x - pad)
											  : 0.0f;
		delta.y = (pos.y < visible_r.Min.y)	  ? (visible_r.Min.y - pos.y + pad)
				  : (pos.y > visible_r.Max.y) ? (visible_r.Max.y - pos.y - pad)
											  : 0.0f;
...
void ImGuiTestContext::ForeignWindowsHideOverPos(ImVec2 pos, ImGuiWindow** ignore_list)
{
	...
	for (int i = 0; ignore_list[i]; i++) min_window_index = ImMin(min_window_index, ImGui::FindWindowDisplayIndex(ignore_list[i]));
...
void ImGuiTestContext::MouseMoveToPos(ImVec2 target)
{
	...
	while (true)
	{
		...
		else
		{
			...
			// ImGui::GetOverlayDrawList()->AddBezierCurve(start_pos, intermediate_pos_a, intermediate_pos_b, target,
...
// TODO: click time argument (seconds and/or frames)
void ImGuiTestContext::MouseDoubleClick(ImGuiMouseButton button) { MouseClickMulti(button, 2); }
...
void ImGuiTestContext::MouseLiftDragThreshold(ImGuiMouseButton button)
{
	...
	g.IO.MouseDragMaxDistanceSqr[button]
		 = (g.IO.MouseDragThreshold * g.IO.MouseDragThreshold) + (g.IO.MouseDragThreshold * g.IO.MouseDragThreshold);
}
...
void ImGuiTestContext::MouseWheel(ImVec2 delta)
{
	...
	while (delta.x != 0.0f || delta.y != 0.0f)
	{
		...
		{
			scroll
				 = ImClamp(scroll, ImVec2(ImMin(delta.x, 0.0f), ImMin(delta.y, 0.0f)), ImVec2(ImMax(delta.x, 0.0f), ImMax(delta.y, 0.0f)));
...
void ImGuiTestContext::GatherItems(ImGuiTestItemList* out_list, ImGuiTestRef parent, int depth)
{
	...
	LogDebug("GatherItems from %s, %d deep: found %d items.", ImGuiTestRefDesc(parent, parent_item).c_str(), depth,
		 end_gather_size - begin_gather_size);
...
void ImGuiTestContext::ItemAction(ImGuiTestAction action, ImGuiTestRef ref, ImGuiTestOpFlags flags, void* action_arg)
{
	...
	else if (action == ImGuiTestAction_Close)
	{
		...
		if ((item->StatusFlags & ImGuiItemStatusFlags_Opened) != 0)
		{
			...
			if ((item->StatusFlags & ImGuiItemStatusFlags_Opened) != 0)
			{
				...
				// FIXME-TESTS: let's not start doing those fuzzy things.. widget should give direction of how to close/open... e.g. do you
				// we close a TabItem?
				if ((item->StatusFlags & ImGuiItemStatusFlags_Opened) != 0)
	...
	else if (action == ImGuiTestAction_Check)
	{
		...
		ItemVerifyCheckedIfAlive(ref,
			 true);	 // We can't just IM_ASSERT(ItemIsChecked()) because the item may disappear and never update its StatusFlags any more!
	}
	else if (action == ImGuiTestAction_Uncheck)
	{
		...
		ItemVerifyCheckedIfAlive(ref,
			 false);  // We can't just IM_ASSERT(ItemIsChecked()) because the item may disappear and never update its StatusFlags any more!
	}
	...
	for (int pass = 0; pass < max_passes; pass++)
	{
		...
		for (int n = scan_start; n != scan_end; n += scan_dir)
		{
			...
			case ImGuiTestAction_Open:
				...
				break;
			case ImGuiTestAction_Close:
				if (item.Depth == highest_depth && (item.StatusFlags & ImGuiItemStatusFlags_Openable)
					 && (item.StatusFlags & ImGuiItemStatusFlags_Opened))
				{
					ItemClose(item.ID);
					actioned_total++;
				}
...
bool ImGuiTestContext::TabBarCompareOrder(ImGuiTabBar* tab_bar, const char** tab_order)
{
	...
	p = buf;
	for (int i = 0; tab_order[i] != NULL; i++) p += ImFormatString(p, buf_end - p, "%s\"%s\"", i ? ", " : " ", tab_order[i]);
...
void ImGuiTestContext::MenuAction(ImGuiTestAction action, ImGuiTestRef ref)
{
	...
	while (path < path_end && !IsError())
	{
		...
		{
			// We cannot move diagonally to a menu item because depending on the angle and other items we cross on our path we could close
			// our target menu. First move horizontally into the menu, then vertically!
			if (depth > 0)
			{
...
void ImGuiTestContext::WindowResize(ImGuiTestRef ref, ImVec2 size)
{
	...
	// Extra yield as newly created window that have AutoFitFramesX/AutoFitFramesY set are temporarily not submitting their resize widgets.
...
void ImGuiTestContext::DockInto(ImGuiTestRef src_id, ImGuiTestRef dst_id, ImGuiDir split_dir, bool split_outer, ImGuiTestOpFlags flags)
{
	...
	Str128f log("DockInto() Src: %s '%s' (0x%08X), Dst: %s '%s' (0x%08X)", node_src ? "node" : "window", node_src ? "" : window_src->Name,
		 node_src ? node_src->ID : window_src->ID, node_dst ? "node" : "window", node_dst ? "" : window_dst->Name,
		 node_dst ? node_dst->ID : window_dst->ID);
	...
	ImVec2 drop_pos;
	bool drop_is_valid
		 = ImGui::DockContextCalcDropPosForDocking(window_dst, node_dst, window_src, node_src, split_dir, split_outer, &drop_pos);
	...
	Yield(2);  // Docking to dockspace over viewport (needs extra frame) or moving a dock node to another node (needs two extra frames)
			   // fails in fast mode without this.
	IM_CHECK_SILENT(
		 g.HoveredWindowUnderMovingWindow && g.HoveredWindowUnderMovingWindow->RootWindowDockTree == window_dst->RootWindowDockTree);

	// Docking will happen on the mouse-up
	const ImGuiID prev_dock_id = window_src->DockId;
	const ImGuiID prev_dock_parent_id
		 = (window_src->DockNode && window_src->DockNode->ParentNode) ? window_src->DockNode->ParentNode->ID : 0;
	...
	if (!(flags & ImGuiTestOpFlags_NoError))
	{
		...
		const ImGuiID curr_dock_parent_id
			 = (window_src->DockNode && window_src->DockNode->ParentNode) ? window_src->DockNode->ParentNode->ID : 0;
		const ImGuiID curr_dock_node_as_host_id = window_src->DockNodeAsHost ? window_src->DockNodeAsHost->ID : 0;
		IM_CHECK((prev_dock_id != curr_dock_id) || (prev_dock_parent_id != curr_dock_parent_id)
				 || (prev_dock_node_as_host_id != curr_dock_node_as_host_id));
	}
}
...
void ImGuiTestContext::PerfCapture(const char* category, const char* test_name, const char* csv_file)
{
	...
	LogInfo("[PERF] Conditions: Stress x%d, %s, %s, %s, %s, %s", PerfStressAmount, build_info->Type, build_info->Cpu, build_info->OS,
		 build_info->Compiler, build_info->Date);
