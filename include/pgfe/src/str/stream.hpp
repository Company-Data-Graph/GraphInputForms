...
std::vector<std::string> read_to_strings_if(std::istream& input, const Pred& pred, const char delimiter = '\n')
{
 ...
 */
inline std::vector<std::string> read_to_strings(std::istream& input, const char delimiter = '\n')
{
	return read_to_strings_if(
		 input, [](const auto&) { return true; }, delimiter);
}
 ...
 * @see read_to_strings_if().
 */
inline std::vector<std::string> read_to_strings(const std::filesystem::path& path, const char delimiter = '\n', const bool is_binary = true)
{
	return read_to_strings_if(
		 path, [](const auto&) { return true; }, delimiter, is_binary);
}
...
std::string read_to_string(std::istream& input, const std::optional<Trim> trim = {})
{
	...
	const auto append_buffer = [&] {
		...
		result.append(buffer.data() + space_count, static_cast<std::size_t>(input.gcount()) - space_count);
	};
	while (input.read(buffer.data(), buffer.size())) append_buffer();
	append_buffer();
 ...
 */
template <std::size_t BufSize = 4096>
Ret<std::string> read_to_string_nothrow(const std::filesystem::path& path, const bool is_binary = true, const std::optional<Trim> trim = {})
{
	...
	if (input)
		...
		return Ret::make_error(Err{Errc::generic, "unable to open \"" + path.generic_string() + "\""});
}
 ...
 */
template <std::size_t BufSize = 4096>
std::string read_to_string(const std::filesystem::path& path, const bool is_binary = true, const std::optional<Trim> trim = {})
{
