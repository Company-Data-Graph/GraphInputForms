...
template <>
struct default_delete<PGnotify> final
{
	void operator()(PGnotify* const ptr) const noexcept { PQfreemem(ptr); }
};

/// The default deleter for `PGresult`.
template <>
struct default_delete<PGresult> final
{
	void operator()(PGresult* const ptr) const noexcept { PQclear(ptr); }
};

/// The default deleter for `PGconn`.
template <>
struct default_delete<PGconn> final
{
	void operator()(PGconn* const ptr) const noexcept { PQfinish(ptr); }
};
...
public:
	...
	explicit operator bool() const noexcept { return static_cast<bool>(pgresult_); }
	...
	PGresult* release() noexcept { return pgresult_.release(); }

	/// @returns The raw pointer to the libpq's result.
	const PGresult* native_handle() const noexcept { return pgresult_.get(); }
	...
	const char* command_tag() const noexcept { return PQcmdStatus(const_cast<PGresult*>(native_handle())); }

	/// @returns The number of rows affected by a SQL command.
	const char* affected_row_count() const noexcept { return PQcmdTuples(const_cast<PGresult*>(native_handle())); }
	...
	const char* er_severity_localized() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_SEVERITY); }

	/// @returns The item of the error report.
	const char* er_severity_non_localized() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_SEVERITY_NONLOCALIZED); }

	/// @returns The item of the error report.
	const char* er_code() const noexcept { return str::coalesce({PQresultErrorField(native_handle(), PG_DIAG_SQLSTATE), "00000"}); }

	/// @returns The item of the error report.
	const char* er_brief() const noexcept { return str::literal(PQresultErrorField(native_handle(), PG_DIAG_MESSAGE_PRIMARY)); }

	/// @returns The item of the error report.
	const char* er_detail() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_MESSAGE_DETAIL); }

	/// @returns The item of the error report.
	const char* er_hint() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_MESSAGE_HINT); }

	/// @returns The item of the error report.
	const char* er_query_position() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_STATEMENT_POSITION); }

	/// @returns The item of the error report.
	const char* er_internal_query_position() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_INTERNAL_POSITION); }

	/// @returns The item of the error report.
	const char* er_internal_query() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_INTERNAL_QUERY); }

	/// @returns The item of the error report.
	const char* er_context() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_CONTEXT); }

	/// @returns The item of the error report.
	const char* er_schema_name() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_SCHEMA_NAME); }

	/// @returns The item of the error report.
	const char* er_table_name() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_TABLE_NAME); }

	/// @returns The item of the error report.
	const char* er_column_name() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_COLUMN_NAME); }

	/// @returns The item of the error report.
	const char* er_data_type_name() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_DATATYPE_NAME); }

	/// @returns The item of the error report.
	const char* er_constraint_name() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_CONSTRAINT_NAME); }

	/// @returns The item of the error report.
	const char* er_source_file() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_SOURCE_FILE); }

	/// @returns The item of the error report.
	const char* er_source_line() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_SOURCE_LINE); }

	/// @returns The item of the error report.
	const char* er_source_function() const noexcept { return PQresultErrorField(native_handle(), PG_DIAG_SOURCE_FUNCTION); }
	 ...
	 */
	int row_count() const noexcept { return PQntuples(native_handle()); }

	/// @returns The number of fields.
	int field_count() const noexcept { return PQnfields(native_handle()); }

	/// @returns `nullptr` if the `position` is out of range.
	const char* field_name(const int index) const noexcept { return PQfname(native_handle(), index); }

	/// @returns `-1` if the given name does not match any field.
	int field_index(const char* const name) const noexcept { return PQfnumber(native_handle(), name); }

	/**
	 * @returns `0` if the `position` is out of range, or if the
	 * corresponding column is not a simple reference to a table column.
	 */
	::Oid field_table_oid(const int position) const noexcept { return PQftable(native_handle(), position); }
	 ...
	 * is positive.
	 */
	int field_table_column(const int position) const noexcept { return PQftablecol(native_handle(), position); }
	 ...
	 */
	Data_format overall_field_format() const noexcept { return to_data_format(PQbinaryTuples(native_handle())); }

	/// @returns The data format of the field.
	Data_format field_format(const int position) const noexcept { return to_data_format(PQfformat(native_handle(), position)); }

	/// @returns The data type OID of the field.
	::Oid field_type_oid(const int position) const noexcept { return PQftype(native_handle(), position); }

	/// @returns `-1` to denote *no information available*.
	int field_type_modifier(const int position) const noexcept { return PQfmod(native_handle(), position); }

	/// @returns `-1` to denote *variable-size*.
	int field_type_size(const int position) const noexcept { return PQfsize(native_handle(), position); }
	...
	int ps_param_count() const noexcept { return PQnparams(native_handle()); }

	/// @returns The data type of a prepared statement parameter.
	::Oid ps_param_type_oid(const int position) const noexcept { return PQparamtype(native_handle(), position); }
	 ...
	 * @returns `true` on success.
	 */
	bool set_attributes(PGresAttDesc* const attributes, const int attribute_count) noexcept
	{
...
/// Result is swappable.
inline void swap(Result& lhs, Result& rhs) noexcept { lhs.swap(rhs); }
