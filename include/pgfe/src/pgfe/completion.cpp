...
DMITIGR_PGFE_INLINE Completion::Completion() noexcept { DMITIGR_ASSERT(!is_valid()); }
		...
		while (space_before_word_pos != std::string_view::npos)
		{
			...
			if (const int err = errno)
				throw Client_exception{
					 "cannot parse command completion tag:"
					 " "
...
DMITIGR_PGFE_INLINE bool Completion::is_valid() const noexcept { return row_count_ > -2; }

DMITIGR_PGFE_INLINE const std::string& Completion::tag() const noexcept { return tag_; }
...
DMITIGR_PGFE_INLINE bool Completion::is_invariant_ok() const noexcept { return (row_count_ < 0) || !tag_.empty(); }
